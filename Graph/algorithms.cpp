...
/* Dijkstra algorithm. While there is an unvisited vertex, select the one with minimum cost and study its neighbors. Stops whrn the current vertex is the destination. */
...
int Graph::algo_ford_fulkerson(const Vertex *source, const Vertex *sink) {
    ...
    *residual_graph          = *this;
    ...
    do {
        ...
        std::cout << "HELL YEAHHHHHH" << std::endl << std::endl << std::endl;
        if(valid_path) {
